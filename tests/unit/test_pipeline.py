...
def test_pipeline_factory_returns_pipeline_context() -> None:
    ...
    ctx = pipeline("run-1", PipelinePolicy(budget=1.00))
...
def test_exit_appends_to_run_log(tmp_path) -> None:
    ...
    ctx = pipeline("run-log-test", PipelinePolicy(budget=1.00), log_path=log_path)
    with ctx:
        pass
...
def test_exit_appends_even_on_exception(tmp_path) -> None:
    ...
    log_path = tmp_path / ".l6e" / "runs.jsonl"
    ctx = pipeline("run-exc-test", PipelinePolicy(budget=1.00), log_path=log_path)
